...
const MONTHS_CONFIG = [
    { label: 'Янв', col: 1 }, { label: 'Фев', col: 5 }, { label: 'Мар', col: 9 },
    { label: 'Апр', col: 14 }, { label: 'Май', col: 18 }, { label: 'Июн', col: 22 },
    { label: 'Июл', col: 27 }, { label: 'Авг', col: 31 }, { label: 'Сен', col: 36 },
    { label: 'Окт', col: 40 }, { label: 'Ноя', col: 44 }, { label: 'Дек', col: 49 },
];

const YearlyChart = () => {
    const sessions = useLogStore((state) => state.sessions);

    const data = useMemo(() => prepareYearlyChartData(sessions), [sessions]);
            ...
            <div className={styles.scrollContainer}>
                <div className={styles.gridWrapper}>
                    ...
                    <div className={styles.monthsGrid}>
                        {MONTHS_CONFIG.map(({ label, col }) => (
                            <span key={label} style={{ gridColumnStart: col }} className={styles.monthLabel}>
                                {label}
                            </span>
                        ))}
                    </div>
                    <div className={styles.daysGrid}>
                        {data.map(({ dateStr, intensity, formattedTime }) => (
                            <div
                                key={dateStr}
                                className={styles.daySquare}
                                style={{ backgroundColor: INTENSITY_COLORS[intensity] }}
                                data-tooltip={`${formatDate(dateStr)}: ${formattedTime}`}
                            />
                        ))}
                    </div>
                </div>
